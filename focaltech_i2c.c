...
int fts_read(u8 *cmd, u32 cmdlen, u8 *data, u32 datalen)
{
    ...
    struct i2c_msg msg_list[2]; // I2C 暫存器讀取的標準做法是「一筆寫位址 + 一筆讀資料」,
    struct i2c_msg *msg = NULL; // 指向真正要傳的訊息起點。純讀時指向第二筆,有命令時指向第一筆。
    int msg_num = 0;            // 這次要傳幾筆訊息(1 或 2)。

    /* must have data when read */
    if (!ts_data || !ts_data->client || !data || !datalen  // !ts_data:主結構還沒初始化; !ts_data->client:I2C 裝置沒綁定; !data:沒給輸出緩衝區。
        || (datalen > I2C_BUF_LENGTH) || (cmdlen > I2C_BUF_LENGTH)) { //超過 rx buffer(256)容量,會溢位; 超過 tx buffer 容量。
        FTS_ERROR("fts_data/client/cmdlen(%d)/data/datalen(%d) is invalid",
                  cmdlen, datalen);
        return -EINVAL; //印錯誤訊息,回傳 -EINVAL(參數錯誤的標準錯誤碼,值為負)。
    }
...
{
    return fts_read(&addr, 1, value, 1); // addr 的位址 (&addr), 因為 fts_read 收的是指標。
}
