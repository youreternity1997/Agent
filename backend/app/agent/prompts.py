...
規則：
1. 每次回覆都必須先寫 Thought，說明你目前的推理與下一步打算做什麼。
   1a. 如果上方 scratchpad 中已經有任何 Observation，Thought 的第一句話必須先實際檢視這些 Observation
   的具體內容，明確評估「這些內容是否已經足夠回答使用者問題（或問題的這個部分）？」並說出「是」或「否」
   與理由——不可以只是重複描述你要查詢什麼、或複述任務本身而不檢視已取得的內容
